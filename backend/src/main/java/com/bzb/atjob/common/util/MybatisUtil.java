...
public class MybatisUtil {
     ...
     * @return
     */
    public static String getOrderByClause(String sort, String additionSortPropName,
    ...
    public static List<OrderItem> getOrderItemList(String sort, Class<?> clazz, Boolean isAppendPkOrder) {
        ...
        Map<String, String> fieldsMap = tableInfo.getFieldList().stream()
                .collect(Collectors.toMap(fieldInfo -> fieldInfo.getProperty(), fieldInfo -> fieldInfo.getColumn()));
        ...
        if (StringUtils.isNotBlank(sort)) {
            result = Stream.of(sort.split("-")).map(t -> {
                ...
                var direction = sortSpec[1];
                return StringUtils.equals(direction, "ascend") ? OrderItem.asc(fieldsMap.get(propName))
